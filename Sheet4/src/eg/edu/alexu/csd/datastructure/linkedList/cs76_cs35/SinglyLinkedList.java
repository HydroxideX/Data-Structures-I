...
public class SinglyLinkedList implements ILinkedList {
    ...
    public SinglyLinkedList sublist(int from, int to) {
        SinglyLinkedList newList = new SinglyLinkedList();
        if (from < 0 || from > getLength(head)-1 || to < 0 || to > getLength(head)-1) {
            System.out.println("Error");
            return newList;
        }
    ...
    public MyListNode getIndex(int index) {
        if (index > getLength(head)-1 || index<0) {
            return null;
        }
        MyListNode current = head;
